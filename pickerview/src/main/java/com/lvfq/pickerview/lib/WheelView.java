...
//        paintCenterText.setTextScaleX(1.1F);
    ...
    private void measuredCenterContentStart(String content) {
        int lableWidth = (label != null ? getTextWidth(paintCenterText, label) : 0);

        Rect rect = new Rect();
        paintCenterText.getTextBounds(content, 0, content.length(), rect);
        switch (mGravity) {
            case Gravity.CENTER:
                drawCenterContentStart = (int) ((measuredWidth - rect.width() - (label != null ? (lableWidth + CENTERCONTENTOFFSET) : 0)) * 0.5);
                break;
            case Gravity.LEFT:
    ...
    private void measuredOutContentStart(String content) {
        ...
        switch (mGravity) {
            case Gravity.CENTER:
                drawOutContentStart = (int) ((measuredWidth - rect.width() - (label != null ? (lableWidth + CENTERCONTENTOFFSET) : 0)) * 0.5);
                break;
            case Gravity.LEFT:
