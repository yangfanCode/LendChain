...
/**
 * 投资全部fragment
 * 1 抵押标 2 平台标预告 3平台标 4新手预告 5新手标
 */
public class InvestAllFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (parentView == null) {
            parentView = inflater.inflate(R.layout.fragment_invest_all, container,
                    false);
            initView();
            initData(true);//不需要每次都刷新数据
            initListener();
        }
    ...
    public static InvestAllFragment newInstance() {
        InvestAllFragment fragment = new InvestAllFragment();
...
//        fragment.setArguments(args);
    ...
    private void initData(boolean isShow) {
        currentPage = 1;
        isRefrensh = true;
        NetApi.investList(getActivity(), isShow, currentPage, Constant.PAGE_SIZE, 0, observer);
    }

    private void loadData() {
        isRefrensh = false;
        NetApi.investList(getActivity(), false, ++currentPage, Constant.PAGE_SIZE, 0, observer);
    }
