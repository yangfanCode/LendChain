 ...
 * 投资抵押标fragment
 */
public class InvestMortGagaFragment extends Fragment {
    @BindView(R.id.refreshLayout)
    SmartRefreshLayout refreshLayout;
    @BindView(R.id.invest_pulltoRefrenshListView)
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (parentView == null) {
            parentView = inflater.inflate(R.layout.fragment_mort_gaga, container,
                    false);
            initView();
            initData(false);
            initListener();
    ...
    public static InvestMortGagaFragment newInstance(){
        InvestMortGagaFragment fragment = new InvestMortGagaFragment();
...
//        listView.setMode(PullToRefreshBase.Mode.BOTH);
    ...
    private void initData(boolean isShow) {
        currentPage=1;
        isRefrensh=true;
        NetApi.investList(getActivity(),isShow,currentPage, Constant.PAGE_SIZE,2,observer);
    }
    private void loadData() {
        isRefrensh=false;
        NetApi.investList(getActivity(),false,++currentPage, Constant.PAGE_SIZE,2,observer);
    }
